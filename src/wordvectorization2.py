...
def get_embedding(text: str, 
    ...
    """
    Generate a normalized embedding for `text` by:
     1) tokenizing and running through BERT
     2) stacking the last 4 hidden layers
     3) mean-pooling across tokens (respecting the attention mask)
     4) L2-normalizing to unit length

    Returns:
      a 1D torch.Tensor (embedding_dim,)
    """
    # 1) tokenize
    inputs = tokenizer(
        text,
    ...
    # 2) forward pass (ask for all hidden states)
    with torch.no_grad():
        outputs = model(**inputs, output_hidden_states=True)

    # 3) stack last 4 layers: shape [4, batch, seq_len, hidden_dim]
    last4 = torch.stack(outputs.hidden_states[-4:], dim=0)

    # 4) build token mask of shape [4, batch, seq_len, 1] 
    mask = (
        inputs['attention_mask']
        .unsqueeze(0)    # for layers dim
        .unsqueeze(-1)   # for hidden_dim
        .expand(last4.size())
        .float()
    )

    # 5) mean-pool: sum over layers & tokens, then divide by count
    summed   = (last4 * mask).sum(dim=(0, 2))               # [batch, hidden_dim]
    counts   = mask.sum(dim=(0, 2)).clamp(min=1e-9)         # [batch, hidden_dim] or [batch,1]
    pooled   = summed / counts                             # [batch, hidden_dim]

    # 6) L2 normalize per example
    normalized = F.normalize(pooled, p=2, dim=1).squeeze(0) # [hidden_dim]
